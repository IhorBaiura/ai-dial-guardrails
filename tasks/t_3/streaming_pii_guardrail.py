...
class PresidioStreamingPIIGuardrail:
    ...
    def __init__(self, buffer_size: int =100, safety_margin: int = 20):
        configuration: dict[str, list[dict[str, str]] | str] = {
    ...
    def process_chunk(self, chunk: str) -> str:
        ...
        if len(self.buffer) > self.buffer_size:
            ...
            for i in range(safe_length - 1, max(0, safe_length - self.safety_margin), -1):
                if self.buffer[i] in ' \n\t.,;:!?':
                    safe_length = i
                    break

            text_to_process = self.buffer[:safe_length]
            results: List[RecognizerResult] = cast(List[RecognizerResult], self.analyzer.analyze(text=text_to_process, language='en'))
            anonymized_text: EngineResult = self.anonymizer.anonymize(text=text_to_process, analyzer_results=results)
    ...
    def finalize(self) -> str:
        ...
        results = cast(List[RecognizerResult], self.analyzer.analyze(text=self.buffer, language='en'))
...
class StreamingPIIGuardrail:
    ...
    def _detect_and_redact_pii(self, text: str) -> str:
        ...
        cleaned_text = text
        for _pattern_name, (pattern, replacement) in self._pii_patterns.items():
...
def main(type: Literal['presidio', 'regex'] = 'presidio'):
    if type == 'presidio':
        guardrails = PresidioStreamingPIIGuardrail(buffer_size=100, safety_margin=20)
    else:
        guardrails = StreamingPIIGuardrail(buffer_size=100, safety_margin=20)
    ...
    while True:
        ...
        for chunk in stream:
            content = cast(str, chunk.content) # type: ignore
            if content:
                safe_chunk = guardrails.process_chunk(content)
                if safe_chunk:
                    print(safe_chunk, end="", flush=True)
                    full_response += safe_chunk

        final_chunk = guardrails.finalize()
        if final_chunk:
